...
private:
//  void append(std::vector< boost::shared_ptr<void> > &payloads, boost::shared_ptr<void> payload) {
//    boost::mutex::scoped_lock slock( my_lock );
...
int cond::TestGTPerf::execute(){
  ...
  for( auto p: proxies ){
	...
	std::cout << "WARNING: Ignoring possibly broken payload of type " << payloadTypeName << std::endl;
	continue;
      }
  ...
  if (nThrD > 1) {
    ...
    //  } else {
    //     pool.size_controller().resize( 0 );
  }
  //  boost::threadpool::pool pool_big( nThrD );
  std::cout << "==> pool size (big): " << pool.size() << std::endl;
  ...
  for( auto p: proxies ){
    ...
    if (nThrD > 1) {
      ...
      // if ( p->getBufferSize() > 100000 ) {
      // 	nBig++;
      // 	pool_big.schedule( boost::bind(&DeserialWorker::run, dw) );
      // } else {
  ...
  // pool_big.wait();
  // if ( !pool_big.empty() ){
  //   std::cerr << "ERROR: thread pool (big) not empty after wait !! active: " << pool_big.active() << " pending " << pool_big.pending() << std::endl;
  // }
  timex.interval("deserializing payloads");

  // pool_big.size_controller().resize(0); // make sure there are no threads any more

  std::cout << "found   " << nEmpty << " empty payloads while deserialising " << std::endl;
  std::cout << "deser'd " << nBig   << " big payloads in the big pool " << std::endl;
