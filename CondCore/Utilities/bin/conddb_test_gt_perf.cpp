...
namespace cond {
  ...
  public:
    ...
    const Binary& getStrInfo() const { return m_strInfo; }
    bool isOra ();
...
cond::UntypedPayloadProxy::UntypedPayloadProxy( const UntypedPayloadProxy& rhs ):
  ...
  m_haveCache( rhs.m_haveCache ), 
  m_isOra_cached( rhs.m_isOra_cached ) {
}
...
bool cond::UntypedPayloadProxy::get( cond::Time_t targetTime, bool debug ){
  ...
  if( targetTime < m_data->current.since || targetTime >= m_data->current.till ){
    ...
    loaded = m_session.fetchPayloadData( m_data->current.payloadId, payloadType, m_buffer, m_strInfo );
    m_session.transaction().commit();
...
bool cond::UntypedPayloadProxy::isOra () { 
  if (m_haveCache) {
    return m_isOra_cached;
  } else {
    m_session.transaction().start();
    m_isOra_cached = m_session.isOraSession();
    m_session.transaction().commit();
    m_haveCache = true;
    return m_isOra_cached;
  }
...
public:
  ...
  void runReal() {
    ...
    auto result = new std::pair< std::string, boost::shared_ptr<void> > (cond::persistency::fetchOne( payloadTypeName, buffer, payloadPtr, p->getStrInfo(), p->isOra() ));
...
///     }
    ...
    if (nThrD > 1) {
      ...
      tasksD.push_back(dw); 
       ...
       try {
	 std::pair<std::string, boost::shared_ptr<void> > result = fetchOne( payloadTypeName, p->getBuffer(), payloadPtr, p->getStrInfo(), p->isOra() );
